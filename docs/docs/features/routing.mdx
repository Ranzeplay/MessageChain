...
- `AbstractRouteExecutor` is a `Function<T, R>` interface.
...
So if it's possible to use the other one, use the other one.
...
Once the request has been sent to server, MessageChain will check if the Route does exist. If not, a `RouteFailResponse` with the reason `FailType.ROUTE_NOT_FOUND` will be sent back.
